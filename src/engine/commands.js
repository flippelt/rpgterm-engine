...
// Audio is provided by the host via ctx.audio (the terminal injects its Web
// Audio sfx; the editor preview and unit tests leave it out). A no-op adapter
// keeps the `volume`/`hum` commands working everywhere. Persistence
// (localStorage in the app) is likewise optional via ctx.persist(key, value).
const NOOP_AUDIO = {
  setVolume() {},
  setMuted() {},
  getVolume: () => 0.4,
...
const audioOf = (ctx) => ctx.audio ?? NOOP_AUDIO
const persist = (ctx, key, value) => ctx.persist?.(key, value)
...
const COMMANDS = {
  ...
  volume: (ctx) => {
      ...
      return [
        {
          text: ctx.t('volume.status', {
            state: audio.isMuted() ? ctx.t('volume.muted') : Math.round(audio.getVolume() * 100) + '%'
          })
        },
    ...
    if (arg === 'mute') {
      audio.setMuted(true)
      persist(ctx, 'tirpg.muted', 'true')
      return [{ text: ctx.t('volume.didmute'), type: 'muted' }]
    }
    if (arg === 'unmute') {
      audio.setMuted(false)
      persist(ctx, 'tirpg.muted', 'false')
    ...
    audio.setVolume(n / 100)
    persist(ctx, 'tirpg.volume', String(n / 100))
  ...
  hum: (ctx) => {
    const audio = audioOf(ctx)
    const arg = ctx.args[0]
    const turnOn = arg === 'on' || (!arg && !audio.isHumOn())
    if (arg && arg !== 'on' && arg !== 'off')
      return [{ text: ctx.t('hum.usage'), type: 'err' }]
    if (turnOn) {
      audio.startHum(ctx.theme.sounds?.hum)
      persist(ctx, 'tirpg.hum', 'on')
      return [{ text: ctx.t('hum.on'), type: 'ok' }]
    }
    audio.stopHum()
    persist(ctx, 'tirpg.hum', 'off')
